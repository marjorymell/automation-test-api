describe('Delete Cart API Testing', () => {
    ...
    it('DELETE (cancel) cart with successs', function() {
        const cartData = {
            produtos: [
                {
    ...
    it('DELETE (cancel) cart for user without', function() {
    cy.fixture('credentials').as('userData').then((userData) => {
    ...
    it('DELETE (cancel) cart with invalid token error', function() {
        const cartId = Cypress.env('cartId');
        Cypress.env('token', null);

        cy.cancelCart(cartId).then((response) => {
